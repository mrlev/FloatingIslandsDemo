...
# Edited to make Godot 4.0 compatible
...
func _ready():
	if camera.enabled:
		...
		mouse.item_selected.connect( self._on_opt_mouse_item_selected )
		...
		freelook.toggled.connect(self._on_btn_freelook_toggled)
		...
		sensitivity.value_changed.connect(self._on_hsb_sensitivity_value_changed)
		...
		smoothness.value_changed.connect(self._on_hsb_smoothness_value_changed)
